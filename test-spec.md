...
| 14 | ディベート終了機能 (異常系) | 1. やり取りが一定回数以下の時チャット画面右上の「終了」ボタンを押す。<br>2. 確認ダイアログで「OK」を押す。 | 「有効なフィードバックが得られない」という旨のエラーメッセージが表示される。 |
| 15 | ディベート終了機能 (異常系) | 1. 一往復以上のやり取りをしていないときにチャット画面右上の「終了」ボタンを押す。 | ボタンが出現していないので押せない。 |
...
| 16 | AIがJSON形式で返さない | `/validate_theme`でAIがJSON形式の応答を返さなかった場合。 | UIにエラーメッセージが表示されるか、処理が停止する。 |
