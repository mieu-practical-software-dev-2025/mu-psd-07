...
| 14 | ディベート終了機能 (正常系) | 1. チャット画面右上の「終了」ボタンを押す。<br>2. 確認ダイアログで「OK」を押す。 | AIがフィードバックを返しディベートが終了する。 |
| 15 | ディベート終了機能 (異常系) | 1. やり取りが一定回数以下の時チャット画面右上の「終了」ボタンを押す。<br>2. 確認ダイアログで「OK」を押す。 | 「有効なフィードバックが得られない」という旨のエラーメッセージが表示される。 |
| 16 | ディベート終了機能 (異常系) | 1. 一往復以上のやり取りをしていないときにチャット画面右上の「終了」ボタンを押す。 | ボタンが出現していないので押せない。 |

### 3.4. API異常系
| No | テスト内容 | 手順 | 期待結果 |
|----|------------|------|----------|
| 17 | `/validate_theme` APIエラー | AIサービスとの通信に失敗した場合。 | UIに「テーマの妥当性チェック中にAIサービスでエラーが発生しました。」等のエラーメッセージが表示される。 |
| 18 | `/send_api` APIエラー | AIサービスとの通信に失敗した場合。 | チャット画面に「エラーが発生しました: ...」というメッセージが表示される。 |
| 19 | AIがJSON形式で返さない | `/validate_theme`でAIがJSON形式の応答を返さなかった場合。 | UIにエラーメッセージが表示されるか、処理が停止する。 |
