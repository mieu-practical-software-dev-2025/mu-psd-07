...
| 14 | ディベート終了機能 (正常系1) | 1. チャット画面右上の「終了」ボタンを押す。<br>2. 確認ダイアログで「OK」を押す。 | AIがフィードバックを返しディベートが終了する。 やり取りが一定回数以下のときには短いコメントが返ってくる。|
| 15 | ディベート終了機能 (正常系2) | 1. チャット画面右上の「終了」ボタンを押す。<br>2. 確認ダイアログで「OK」を押す。 | AIがフィードバックを返しディベートが終了する。 やり取りが一定回数より多いときには点数付きのフィードバックが返ってくる。|
| 16 | ディベート終了機能 (異常系1) | 1. やり取りが一定回数以下などの時チャット画面右上の「終了」ボタンを押す。<br>2. 確認ダイアログで「OK」を押す。 | 「有効なフィードバックが得られない」という旨のエラーメッセージが表示される。 |
| 17 | ディベート終了機能 (異常系2) | 1. 一往復以上のやり取りをしていないときにチャット画面右上の「終了」ボタンを押す。 | ボタンが出現していないので押せない。 |

### 3.4. API異常系
| No | テスト内容 | 手順 | 期待結果 |
|----|------------|------|----------|
| 18 | `/validate_theme` APIエラー | AIサービスとの通信に失敗した場合。 | UIに「テーマの妥当性チェック中にAIサービスでエラーが発生しました。」等のエラーメッセージが表示される。 |
| 19 | `/send_api` APIエラー | AIサービスとの通信に失敗した場合。 | チャット画面に「エラーが発生しました: ...」というメッセージが表示される。 |
| 20 | AIがJSON形式で返さない | `/validate_theme`でAIがJSON形式の応答を返さなかった場合。 | UIにエラーメッセージが表示されるか、処理が停止する。 |

## 4. その他
- 日本語以外の入力対応（返答は常に日本語）
- クロスブラウザテスト**: Chrome, Edgeで主要な機能が同様に動作することを確認する。
